...
static Ref_t create_detector(Detector& theDetector, xml_h element, SensitiveDetector sens) {
  ...
  printout(dd4hep::DEBUG, "building LUXEEcalPrototype", "Ecal_dim_x : %e    Ecal_dim_y: %e    Ecal_dim_z: %e ",
      ...
      for (xml_coll_t k(x_layer, _U(slice)); k; ++k) {
        ...
        if (sliceHasModules) {
          for (xml_coll_t q(x_slice, _U(module)); q; ++q) {
            ...
            // Module volume & box
            Volume module_vol(module_name, Box(0.5 * module_sizeX, 0.5 * module_sizeY, slice_thickness / 2),
                              slice_material);
            ...
            PlacedVolume module_phv =
                slice_vol.placeVolume(module_vol, Position(0 + module_offsetX, 0 + module_offsetY, 0));
